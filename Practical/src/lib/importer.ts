...
export async function detectAnomalies(row: ParsedCSVRow, groupId: string) {
  ...
  const dbUserNames = dbUsersList.map(u => u.name);
  ...
  if (parsedDate) {
    ...
    if (resolvedPayer) {
      const payerObj = dbUsersList.find(u => u.name === resolvedPayer);
    ...
    resolvedSplitWith.forEach(memberName => {
      const memberObj = dbUsersList.find(u => u.name === memberName);
  ...
  if (parsedDate && resolvedPayer && !isNaN(numericAmount)) {
    ...
    const dbDuplicate = await prisma.expense.findFirst({
      where: {
        ...
        paidById: dbUsersList.find(u => u.name === resolvedPayer)?.id,
...
export async function importCSVToStaging(csvContent: string, groupId: string) {
  ...
  for (const row of parsedRows) {
    ...
    if (startOfRowDate) {
      ...
      const isDuplicateInFile = results.some(r => {
...
export async function convertStagedToExpense(stagedId: string, resolvedValuesOverride?: any) {
  ...
  const payerUser = dbUsers.find(u => u.name === val.payer);
  ...
  if (isSettlement) {
    ...
    const receiverUser = dbUsers.find(u => u.name === receiverName);
    if (receiverUser) {
    ...
    for (const name of splitMembers) {
      const user = dbUsers.find(u => u.name === name);
      if (user) {
        await prisma.expenseSplit.create({
          data: {
            expenseId: expense.id,
    ...
    for (const part of parts) {
      const match = part.match(/(.+)\s+(\d+)\s*%/);
      if (match) {
        const name = resolveUserName(match[1], dbUsers.map(u => u.name));
    ...
    for (const item of parsedShares) {
      const user = dbUsers.find(u => u.name === item.name);
      if (user) {
