...
export async function resolveStagedExpenseAction(
  ...
  try {
    ...
    const dbUserNames = dbUsers.map(u => u.name);
    ...
    // Check payer
    const payerObj = dbUsers.find(u => u.name === resolvedPayer);
    ...
    for (const name of resolvedSplitWith) {
      const uObj = dbUsers.find(u => u.name === name);
...
export async function executeSettlementAction(fromName: string, toName: string, amount: number, currency: string) {
  try {
    const dbUsers = await prisma.user.findMany();
    const fromUser = dbUsers.find(u => u.name === fromName);
    const toUser = dbUsers.find(u => u.name === toName);
